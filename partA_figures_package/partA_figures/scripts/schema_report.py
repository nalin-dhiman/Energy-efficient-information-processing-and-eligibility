...
def check_consistency(provenance_path, data_report):
    """Checks for consistency between provenance (QC thresholds) and data."""
    warnings = []
    ...
    # Example check: if provenance mentions a min_weight threshold, check if it holds.
    # Note: This is heuristic based on common fields.
    
    return warnings

def generate_schema_report(data_dir, out_dir):
    """Reads all parquets, checks schema, counts, and missingness."""
    ...
    for p_file in parquet_files:
        ...
        try:
            ...
            if 'retinotopy' in basename:
                # auto-detect coordinate columns if standard names aren't present
                cols = df.columns
    ...
    # Check consistency with provenance
