#!/usr/bin/env python3
"""
Part D — Publication-ready figures & tables.

This script is intentionally "boring and reliable": it only reads canonical CSV outputs
and renders publication-quality plots (vector PDF + 600-dpi PNG) with non-overlapping
legends and large, readable typography.
...
Optional:
- stagewise_decoder_results_energywiring.csv (supplementary)

All MI values are assumed to be in bits (base-2). Global MI is expected to include
`mi_low` / `mi_high` bounds if available.
"""
...
def configure_matplotlib() -> None:
    mpl.rcParams.update({
        ...
        "lines.linewidth": 2.0,
        "patch.linewidth": 1.0,
        "pdf.fonttype": 42,   # embed fonts for Illustrator
        "ps.fonttype": 42,
    })
...
def bootstrap_mean_ci(x: np.ndarray, n_resamples: int = 10000, confidence_level: float = 0.95, seed: int = 0) -> Tuple[float, float]:
    """
    Percentile bootstrap CI for the mean (uses scipy.stats.bootstrap, BCa by default).
    """
...
def parse_bin_columns(df_local: pd.DataFrame) -> pd.DataFrame:
    """
    Ensure df_local has integer columns bin_u and bin_v.
    Accepts either:
      - existing columns bin_u, bin_v
      - a single string column 'bin' formatted like '(u,v)'
    """
    df = df_local.copy()
...
def make_tables(energy: pd.DataFrame, global_info: pd.DataFrame, local_info: pd.DataFrame, sens: pd.DataFrame, tables_dir: Path) -> Dict[str, pd.DataFrame]:
    ...
    # Table D1: energy components + totals
    e = energy.copy()
    ...
    # Table D2: information summary + local CI
    g = global_info.copy()
    ...
    # Table D3: sensitivity ratios vs Null_Conn
    s = sens.copy()
...
def figD1_energy_components(energy: pd.DataFrame, out_dir: Path) -> None:
    ...
    ax.legend(loc="lower center", bbox_to_anchor=(0.5, -0.28), frameon=False, title="Component", ncol=4)
...
def figD2_info_and_efficiency(global_info: pd.DataFrame, local_info: pd.DataFrame, sens: pd.DataFrame, out_dir: Path) -> None:
    ...
    bar_colors = {"global": "#4C78A8", "local": "#F58518"}
...
def figD3_local_mi_maps(local_info: pd.DataFrame, out_dir: Path) -> None:
    ...
    fig, axes = plt.subplots(1, ncond, figsize=(4.8*ncond + 2.1, 5.8), constrained_layout=True, sharex=True, sharey=True)
    if ncond == 1:
        axes = [axes]
...
def figD4_efficiency_sensitivity(sens: pd.DataFrame, out_dir: Path) -> None:
    ...
    fig, axes = plt.subplots(2, len(gamma_vals), figsize=(4.8*len(gamma_vals) + 1.4, 7.8),
    ...
    for j, gamma in enumerate(gamma_vals):
        ...
        for bi in range(len(beta_vals)):
            for ai in range(len(alpha_vals)):
                val = grid_g[bi, ai]
                if np.isfinite(val):
                    axg.text(ai, bi, f"{val:.2f}", ha="center", va="center", fontsize=8,
        ...
        for bi in range(len(beta_vals)):
            for ai in range(len(alpha_vals)):
                val = grid_l[bi, ai]
                if np.isfinite(val):
                    axl.text(ai, bi, f"{val:.2f}", ha="center", va="center", fontsize=8,
                             color="white" if val > 1.25 else "black")
...
def maybe_fig_stagewise(data_dir: Path, out_dir: Path) -> None:
    """
    Optional supplementary plot, if a stage-wise decoding CSV is present.

    Accepts either:
      - data/stagewise_decoder_results.csv with columns [Stage, Model, MI_bits]
      - (legacy) data/stagewise_decoder_results_energywiring.csv with [stage, Model, mi_bits]

    The plot is not required for the core Part D story; it is included as a convenience.
    """
    ...
    df = pd.read_csv(path)
    df["Model"] = df["Model"].replace({"Null_Shuff": "Null_Strength"})
    ...
    # Order stages if they look like T4/T5/L_Out/Global; otherwise use sorted order.
    ...
    model_order = ["Real", "Null_Conn", "Null_Strength"]
    models = [m for m in model_order if m in df["Model"].unique()] + [m for m in df["Model"].unique() if m not in model_order]
    ...
    ax.legend(loc="lower center", bbox_to_anchor=(0.5, -0.28), frameon=False, ncol=len(models))
...
def main() -> None:
    ...
    print("[OK] Wrote tables to:", tables_dir)
