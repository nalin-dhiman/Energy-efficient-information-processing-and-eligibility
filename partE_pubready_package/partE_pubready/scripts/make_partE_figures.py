...
def figE1_patch_generalization(data_dir: str, out_dir: str, tables_dir: str) -> None:
    ...
    for i, (wr, k) in enumerate(zip(wt["win_rate"], wt["wins"])):
        if wr > 0.18:
            axC.text(i, wr * 0.55, f"{k}", ha="center", va="center", fontsize=9, color="white", fontweight="bold")
        else:
            axC.text(i, min(1.02, wt["win_rate_hi"].iloc[i] + 0.04), f"{k}", ha="center", va="bottom", fontsize=9)
...
def figE2_dualbaseline(data_dir: str, out_dir: str, tables_dir: str) -> None:
    ...
    ax.legend(loc="lower center", bbox_to_anchor=(0.5, -0.28), frameon=False, ncol=4)
...
def figE4_compute_fairness(data_dir: str, out_dir: str, tables_dir: str) -> None:
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(df["label"], rotation=32, ha="right")
    ...
    # Add numeric values
    span = df["D_3z"].max() - df["D_3z"].min() + 1e-9
    for i, v in enumerate(df["D_3z"].values):
        ax.text(i, v + 0.03 * span, f"{v:.2f}", ha="center", va="bottom", fontsize=9)
    ...
    ax.legend(handles=handles, loc="upper center", bbox_to_anchor=(0.5, 1.18), frameon=False, ncol=4)
...
def figE5_scale_robustness(data_dir: str, out_dir: str, tables_dir: str) -> None:
    ...
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12.8, 4.8), gridspec_kw={"wspace": 0.35}, layout="constrained")
    ...
    fig.legend(handles, labels, loc="upper center", bbox_to_anchor=(0.5, 1.02), frameon=False, ncol=4)
...
def figE6_failure_policy_sensitivity(data_dir: str, out_dir: str, tables_dir: str) -> None:
    ...
    fig, ax = plt.subplots(figsize=(9.6, 3.8), layout="constrained")
